...
class Level:
    ...
    def encounter(self, hero):
        if self.type == "empty":
            print("\nВы тихонько приоткрываете дверь и прислушиваетесь к темноте. Звуков не слышно. Рискнув, вы вытягиваете руку с факелом, освещая пространство за дверью. Фух, комната оказывается пустой, можно продолжать путь.")
        elif self.type == "treasure":
            print("\nВы прижимаетесь ухом к двери и ничего не слышите. Надеясь, что вас не поджидает кровожадный монстр, вы открываете дверь и освещаете комнату. Внутри никого.")
            ...
            print("\nЗабыв об осторожности, вы открываете дверь, не прислушиваясь. Факел освещает комнату и вы видите монстра, сидящего в центре.")
            var = random.randint(1, 3)
            if var == 1:
            ...
            if var == 1:
                print("\nЗа дверью слышится какое-то журчание. Неужели вода? Вы осторожно заходите внутрь и видите, что из под земли бьёт целебный источник. Вам несказанно повезло! Целебная вода стремительно залечивает ваши раны.")
                hero.heal(30)
            else:
                a = input("\nПространство за дверью отвечает тишиной и вы осторожно заходите внутрь. В комнате стоит постамент с бутылочкой необычной формы. Рискнёте выпить? 1. Да. 2. Нет.\nВыберите вариант ответа (1-2):  ")
                if a == "1":
                    if random.random() < 0.8:
            ...
            print("\nПодходя к двери, вам даже не нужно стараться, чтобы услышать чей-то голос. Похоже, что кто-то разговаривает сам с собой. Вы рискуете зайти, ведь другого пути нет.")
...
def run():
    ...
    while hero.alive and hero.level < 100:
        ...
        if hero.shoes == True:
            if hero.level == 25 or hero.level == 26:
                print("\nЗа следующей дверью слышится шелест. «Неужели ветер?» - думаете вы и приоткрываете дверь.")
                ...
                if random.random() < 0.7:
                    print("\nОткрывая следующую дверь, вы видите тролля немаленьких размеров. Он сидит напротив входа в другую комнату, не давая пройти. Вы узнаёте его - это алчный тролль, который питается золотом.")
                    ...
                    while explode == False and money_enough == True:
                        if hero.gold < 10:
                            ...
                            if random.random() < 0.4:
                                ...
                                print("Вы группируетесь, закрывая голову, ведь на вас сыпется настоящий золотой дождь из монет! Вы собираете все монеты, которые съел тролль, включаю те, что кинули ему ранее. Да вы теперь самый настоящий богач!")
                                hero.gold_add(150)
                                print("Помимо монет вы замечаете лежащий на полу пояс. Решив, что не стоит упускать такую возможность вы надеваете украшение на себя.")
                                hero.midas = True
                    ...
                    print("\nОткрывая следующую дверь, вы едва можете войти в неё: почти всё пространство занимает огромная туша тролля. Вы узнаёте его - это алчный тролль, который питается золотом.")
                ...
                print("\nПодходя к следующей двери, вы слышите едва уловимый звук и не можете даже предположить, кто может его издавать. За неимением других вариантов вам приходится открыть дверь.")
    ...
    if hero.level >= 100:
        ...
        input("Для продолжения нажмите Enter...")
        dragon = 100
        print("\nОткрывая следующую дверь, вы ожидаете увидеть очередную комнату, но перед вами роскошный зал. Стены украшены барельефами, мраморный пол начищен до блеска, по центру стоит бело-золотой сундук.")
        ...
        if f == "1":
            ...
            print("Обойдя сунудк, вы решительно направляетесь к большим воротам, отделяющим вас от финальной комнаты и того, что бы там ни было.")
            print("За воротами оказывается огромная сокровищница: пол усеян горами золотых монет, колонны, подпирающие стены, тоже, кажется, сделаны из золота. Посреди этого ослепляющего безумства сидит огромный чёрный дракон!")
            print("Вы входите. Дракон сразу замечает вас и расправляет крылья, которые закрывают половину зала. Вы молча смотрите гиганту в глаза, готовясь к бою, он - смотрит в ответ.")
        ...
        while dragon > 0:
            ...
            if final == 1:
                print(f"Здоровье дракона: {dragon} | Ваше здоровье: {hero.health}")
